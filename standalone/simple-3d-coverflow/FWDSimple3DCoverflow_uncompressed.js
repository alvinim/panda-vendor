...
var FWDS3DCovCategoriesButton = function(
		...
		this.setupMainContainers = function()
		{
			self.bkDO = new FWDSimpleDisplayObject("div");
			...
			self.dumyDO = new FWDSimpleDisplayObject("div");
			if(FWDUtils.isIE){
		...
		this.setSelectedState = function(animate){
			if(animate){
				TweenMax.to(self.textDO.screen, .6, {css:{color:self.textSelectedColor_str}, ease:Quart.easeOut});
			}else{
		...
		this.setNormalState = function(animate){
			if(animate){
				TweenMax.to(self.textDO.screen, .6, {css:{color:self.textNormalColor_str}, ease:Quart.easeOut});
			}else{
		...
		this.destroy = function(){
			...
			TweenMax.killTweensOf(self.textDO.screen);
			TweenMax.killTweensOf(self.bkDO.screen);
	...
	FWDS3DCovCategoriesButton.setPrototype = function(){
		FWDS3DCovCategoriesButton.prototype = new FWDDisplayObject("div");
	};
...
(function(window)
{
	var FWDS3DCovCategoriesMenu = function(parent, labelsAr, curId, maxWidth, menuHeight, catOffset, catColorN, catColorS)
	{
		...
		this.init = function()
		{
			...
			self.buttonsHolderDO = new FWDDisplayObject("div");
		...
		this.onMouseMove = function(e)
		{
			...
			TweenMax.to(self.buttonsHolderDO, .2, {x:Math.floor(newX)});
		};
	...
	{
		FWDS3DCovCategoriesMenu.prototype = new FWDDisplayObject3D("div");
	};
...
(function(window)
{
	var FWDS3DCovData = function(props)
	{
		...
		this.parseProperties = function()
		{
			...
			self.gradientDir = self.propsObj.thumbnailGradientDirection;
			...
			for (var i=0; i<totalDataLists; i++)
			{
				...
				for (var j=0; j<totalChildren; j++)
				{
					...
					if (FWDUtils.hasAttribute(mediaKid, "data-thumbnail-width") && FWDUtils.hasAttribute(mediaKid, "data-thumbnail-height"))
					{
						obj.thumbWidth = parseInt(FWDUtils.trim(FWDUtils.getAttributeValue(mediaKid, "data-thumbnail-width")));
						obj.thumbHeight = parseInt(FWDUtils.trim(FWDUtils.getAttributeValue(mediaKid, "data-thumbnail-height")));
					}
					else
					{
	...
	{
		FWDS3DCovData.prototype = new FWDEventDispatcher();
	};
...
(function(window)
{
	...
	{
		FWDS3DCovScrollbar.prototype = new FWDDisplayObject("div");
	};
...
(function(window)
{
	...
	{
		FWDS3DCovSlideshowButton.prototype = new FWDDisplayObject("div");
	};
...
(function(window)
{
	var FWDS3DCovThumb = function(id, data, parent)
	{
		...
		{
			self.mainDO = new FWDDisplayObject("div", "absolute", "visible");
		...
		{
			if (self.isMobile || FWDUtils.isIEAndLessThen10 || !parent.showGradient)
				return;
			...
			if (parent.showRefl && !self.isMobile && !FWDUtils.isIEAndLessThen10)
			{
			...
			self.gradientLeftDO = new FWDSimpleDisplayObject("div");
			...
			self.gradientLeftDO.setCSSGradient(parent.gradientDir, parent.gradientColor1, parent.gradientColor2);
			
			self.gradientRightDO = new FWDSimpleDisplayObject("div");
			...
			{
				self.gradientRightDO.setCSSGradient(parent.gradientDir, parent.gradientColor1, parent.gradientColor2);
			}
			else
			{
				self.gradientRightDO.setCSSGradient(parent.gradientDir, parent.gradientColor2, parent.gradientColor1);
			}
			...
			if (!self.isMobile && !FWDUtils.isIEAndLessThen10 && parent.showGradient)
			{
				switch (self.gradPos)
				{
					case 0:
						...
						TweenMax.to(self.gradientRightDO, .8, {alpha:0, onComplete:self.hideGrad});
						break;
					case 1:
		...
		this.showThumbIntro2D = function(scale, del)
		{
			...
			if (!FWDUtils.hasTransform2d)
			{
				...
				{
					TweenMax.to(self, .8, {x:Math.floor(self.newX + (self.thumbWidth - scaleW)/2), y:(-Math.floor(parent.thumbsMaxHeight/2) + (parent.thumbsMaxHeight - self.thumbHeight)) * scale, ease:Expo.easeOut});
				}
				else
				{
					TweenMax.to(self, .8, {x:Math.floor(self.newX + (self.thumbWidth - scaleW)/2), y:-Math.floor(scaleH/2), ease:Expo.easeOut});
				}
			}
			else
			{
				...
				TweenMax.to(self, .8, {x:self.newX, y:self.newY, scale:self.thumbScale, delay:del, ease:Quart.easeOut, onComplete:self.setThumbVisibility});
			}
		};
		...
		this.setScale = function(scale, alpha)
		{
			...
			if (!FWDUtils.hasTransform2d)
			{
				...
				if (data.thumbsAlignment == "bottom")
				{
					TweenMax.to(self, .8, {x:Math.floor(self.newX + (self.thumbWidth - scaleW)/2), y:(-Math.floor(parent.thumbsMaxHeight/2) + (parent.thumbsMaxHeight - self.thumbHeight)) * scale, w:scaleW, h:scaleH, alpha:alpha, ease:Quart.easeOut});
				}
				else
				{
			...
			{
				TweenMax.to(self, .8, {x:Math.floor(self.newX), y:Math.floor(self.newY), scale:self.thumbScale, alpha:alpha, ease:Quart.easeOut, onComplete:self.setThumbVisibility});
			}
		};
		...
		this.updateRefl = function(sr)
		{
			if (sr)
			{
				...
				{
					TweenMax.to(self.reflCanvasDO, .8, {alpha:1, ease:Quart.easeOut});
					self.reflCanvasDO.setY(self.thumbHeight + parent.reflDist);
				}
			}
			else
			{
				if (self.reflCanvasDO)
				{
					TweenMax.to(self.reflCanvasDO, .8, {alpha:0, ease:Quart.easeOut});
				}
			}
		};
		...
		this.onMouseOverHandler = function(e)
		{
			if (!self.isOver2 && (self.id != parent.curId))
			{
				...
				TweenMax.to(self, .6, {z:self.curZ + parent.thumbHoverOffset, ease:Expo.easeOut});
			}
		};
	...
	{
		FWDS3DCovThumb.prototype = new FWDDisplayObject3D("div", "absolute", "visible");
	};
...
(function(window)
{
	var FWDS3DCovThumbsManager = function(data, parent)
	{
		...
		this.gradientDir = data.gradientDir;
		...
		this.resizeHandler = function()
		{
			...
			if (self.textDO)
			{
				...
				self.textDO.setY(Math.floor(self.stageHeight/2 + self.thumbHeight/2 + self.textOffset));
			}
		};
		...
		this.addThumbText = function()
		{
			if (self.textDO)
			{
				...
				TweenMax.killTweensOf(self.textDO);
				TweenMax.to(self.textDO, .8, {alpha:1, ease:Expo.easeOut});
			}
		};
		...
		{
			TweenMax.killTweensOf(self.textDO);
			TweenMax.to(self.textDO, .8, {alpha:0, ease:Expo.easeOut});
		};
		...
		this.setupIntro3D = function()
		{
			...
			for (var i=0; i<self.totalThumbs; i++)
			{
					...
					case "acordeon":
						if (i != self.curId)
						{
							if ((self.nrThumbsToDisplay > 0) && (Math.abs(i - self.curId) <= self.nrThumbsToDisplay))
							{
								...
								{
									newX = self.thumbWidth*3/8 * Math.abs(i - self.curId) * sgn;
									newZ = -self.thumbWidth * Math.sqrt(3)/8 * Math.abs(i - self.curId);
									
									newAngleY = 0;
								}
								else
								{
									newX = (self.thumbWidth*3/8 * (Math.abs(i - self.curId)-1) + self.thumbWidth*3/8) * sgn;
									newZ = -self.thumbWidth * Math.sqrt(3)/8 * Math.abs(i - self.curId);
							...
							else
							{
								if (self.nrThumbsToDisplay > 0)
								{
									...
									if (Math.abs(i - self.curId) % 2 == 0)
									{
										newX = self.thumbWidth*3/8 * (self.nrThumbsToDisplay + 1) * sgn;
										newZ = -self.thumbWidth * Math.sqrt(3)/8 * (self.nrThumbsToDisplay + 1);
										
										newAngleY = 0;
									}
									else
									{
										newX = (self.thumbWidth*3/8 * ((self.nrThumbsToDisplay + 1)-1) + self.thumbWidth*3/8) * sgn;
										newZ = -self.thumbWidth * Math.sqrt(3)/8 * (self.nrThumbsToDisplay + 1);
				...
				TweenMax.to(thumb, .8, {x:Math.floor(newX), y:Math.floor(newY), z:Math.floor(newZ), angleX:newAngleX, angleY:newAngleY, alpha:newAlpha, delay:delay/1000, ease:Quart.easeOut});
		...
		this.gotoThumb3D = function()
		{
			...
			for (var i=0; i<self.totalThumbs; i++)
			{
				...
				TweenMax.killTweensOf(thumb);
				TweenMax.to(thumb, .8, {x:Math.floor(newX), y:Math.floor(newY), z:Math.floor(newZ), angleX:newAngleX, angleY:newAngleY, alpha:newAlpha, ease:Quart.easeOut});
		...
		this.sortZ = function()
		{
			...
			if ((s > self.totalThumbs/2) && (self.topology != "flipping") && (FWDUtils.hasTransform3d && !self.data.showDisplay2DAlways))
			{
				for (var i=0; i<self.totalThumbs; i++)
				{
					thumb = self.thumbsAr[i];
		...
		{
			self.controlsDO = new FWDDisplayObject3D("div");
		...
		this.showControls = function()
		{
			if (self.data.controlsPos)
			{
				TweenMax.to(self.controlsDO, .8, {y:self.controlsOffset, ease : Expo.easeOut});
			}
			else
			{
				TweenMax.to(self.controlsDO, .8, {y:self.stageHeight - self.controlsHeight - self.controlsOffset, ease : Expo.easeOut});
			}
		};
		...
		this.setPreset = function(e)
		{
			...
			TweenMax.to(self.thumbsHolderDO, .8, {angleX:e[10], angleY:e[11], ease:Quart.easeOut});
			
			self.topology = e[12];
			self.gradientDir = e[13];
	...
	{
		FWDS3DCovThumbsManager.prototype = new FWDDisplayObject3D("div");
	};
...
(function(window)
{
	var FWDSimple3DCoverflow = function(props)
	{
		...
		this.resizeHandler = function()
		{
			...
			else if (self.displayType == FWDSimple3DCoverflow.AFTER_PARENT)
			{
			...
			if (self.preloaderLoaded)
			{
				if (self.propsObj.backgroundImagePath)
				{
					if (self.displayType == FWDSimple3DCoverflow.RESPONSIVE)
					{
		...
		{
			self.disableDO = new FWDDisplayObject3D("div");
	...
	FWDSimple3DCoverflow.AFTER_PARENT = "afterparent";
...
(function (window){
	...
	FWDUtils.isIE = (function(){
		...
		return isIE || document.documentElement.msRequestFullscreen;
	}());
